...
describe("GET /social/posts/:id/view-series", () => {
  ...
  it("treats unknown window values as `day`", async () => {
    // The route narrows `req.query.window` against the known set
    // ("day"/"week"/"month"); anything else (typos, garbage) must fall back
    // to the 24h daily view.
    state.posts.set(1, { id: 1, playerId: 1, viewCount: 0 });
    state.currentPlayerId = 1;

    const res = await fetch(`${baseUrl}/social/posts/1/view-series?window=bogus`);
