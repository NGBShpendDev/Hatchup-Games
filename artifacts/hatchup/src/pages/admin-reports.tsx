...
function AdminReportsInner() {
  ...
  const handleSuspend = async (reportedUserId: number, reportId: number) => {
    ...
    let reason = "";
    // Reason is required so future admins (and appeals) have an audit trail.
    // Loop until the admin either supplies a non-empty reason or cancels.
    while (true) {
      const reasonInput = window.prompt(
        `Reason for suspending account #${reportedUserId}? (Required — shown to the player and on the suspended-users list. Max 500 chars.)`,
        reason,
      );
      if (reasonInput === null) {
      ...
      reason = reasonInput.trim().slice(0, 500);
      if (reason) break;
