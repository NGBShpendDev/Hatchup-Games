...
- **Player privacy data** — profile data, blocked-user relationships, moderation reports, location visibility settings, family-group membership, weekly activity, and encrypted coordinates. Exposure can reveal sensitive behavior, relationship, or location information.
...
- **API to external services** — Stripe, Clerk, Nominatim, and health-provider OAuth callbacks/webhooks are separate trust domains. Signatures, callback state, redirect assumptions, and any client-originated health-sync claims must be validated.
...
- **Highest-risk code areas:** auth/admin gates, social/privacy routes, location + family/group features, club/challenge membership gates, storage/object ACL flows, health-sync OAuth callbacks plus Apple Health client-push ingestion, Stripe/webhook + entitlement logic
- **Public surfaces:** social post detail/view endpoints, event/club/leaderboard public reads, OG routes, selected webhook/callback endpoints
...
The main risk is broken access control across user-to-user and user-to-admin boundaries. Routes that accept object paths, player IDs, group IDs, family-group IDs, club IDs, challenge IDs, or battle identifiers MUST verify that the caller is the owner, a member, an invitee, or an authorized admin before exposing data or mutating state. Admin-only functions MUST not be reachable through client-controlled flags or partially protected helper endpoints. Query construction and object path handling MUST not let user input escalate into arbitrary database access, unauthorized file access, or privilege changes.